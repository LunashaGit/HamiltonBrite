    ...
    <x-slot name="content">
        <section class="flex flex-col pt-36 pb-24 bg-silver dark:bg-silver-blue">
            <div
                    ...
                    <form class="" method="POST" action="/profile/update/{{ request()->user()->id }}"
                        ...
                        <div>
                            ...
                            <textarea placeholder="About me" rows="5" cols="40" name="bio" id="bio" value="{{ request()->user()->bio }}"
                                class="box-border block w-full h-40 text-sm border rounded-lg resize-none focus:ring-blue-500 p-2.5 focus:border-blue-500 bg-gray-600 border-gray-500 placeholder-gray-400text-white">{{ request()->user()->bio }}</textarea>
                        </div>
                ...
                <div class="p-2 mt-4 flex items-end">
                    <form class="flex float-right" method="post" action="/profile/delete/{{ request()->user()->id }}">
                        @csrf
                        @method('DELETE')
                        <a class="text-sm hover:transition hover:duration-200 text-whitesmoke hover:scale-110 hover:text-red-600 ">
